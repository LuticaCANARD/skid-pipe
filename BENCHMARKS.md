...
| Width | 100-stage success | 100-stage first error | Run future | `.text`, v7em |
|---:|---:|---:|---:|---:|
| 4 | 517.25 ns | 54.603 ns | 608 B | 6,651 B |
| 8 | 466.66 ns | 35.200 ns | 320 B | 6,721 B |
| 16 | 386.30 ns | 23.370 ns | 176 B | 6,101 B |

Wider won every row, so width is a value to pick rather than a knob to expose —
and a Cargo feature would be the wrong mechanism anyway, since features must be
additive and `width-4` beside `width-16` is not. Sixteen is what ships, which is
why the ladder is generated by a macro: writing 34 impls per trait out by hand
is what kept the width at eight.
...
| Target | 0.2.1 | 0.3.0 |
|---|---:|---:|
| `thumbv7em-none-eabihf` | 10,375 B | 6,045 B |
